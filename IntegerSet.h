...
public:
    ...
    // Comment 1: Return an new union (new IntegerSet) of two sets, so the return data type should be IntegerSet.
    ...
    // Comment 2: Check if all elements of two sets are equal to each other. The const at the end
    //            ensures the current object (the one calling the function) will not be modified.
    bool isEqualTo(const IntegerSet &set2) const;
    void inputSet();

    // Comment 3: Additional constructor that receives an array of integers and its size.
